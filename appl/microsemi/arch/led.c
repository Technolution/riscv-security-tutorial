...
void setLeds(int state, int mask){
	...
	//printf("[LEDS]  state 0x%08x, mask = 0x%08x, write 0x%08x\n", state, mask, outputs);
	GPIO_set_outputs(&g_gpio0, outputs);
	/* end of critical section */
}

/**
 * commit the led state to the actual outputs. This allows to change the led
 * values all at once.
 *
 */
void writeLeds(void){
	GPIO_set_outputs(&g_gpio0, led_state << LED_SHIFT);
